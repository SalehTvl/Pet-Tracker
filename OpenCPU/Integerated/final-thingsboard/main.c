...
#include "ril_gps.h"
#include <math.h> // Added for distance calculations
...
float g_lat = 0.0;
float g_lon = 0.0;
float g_speed = 0.0; // Still used for parsing RMC safely, but ignored in logic
bool  g_gps_powered = FALSE;

// --- New Vars for Logic ---
float g_pet_weight = 5.0; // Default weight (kg)
float g_cot_c = 3.0;      // Default Cost of Transport constant (e.g., ~3 for dogs)
int   g_mode = 3;         // Default: 3 (10 seconds)

// Variables for Distance Calculation
float g_prev_lat = 0.0;
float g_prev_lon = 0.0;
bool  g_is_first_fix = TRUE; // To handle the first point logic
float g_interval_calories = 0.0; // Store calories ONLY for the current interval
...
static float Calculate_Distance_Meters(float lat1, float lon1, float lat2, float lon2);
static void Calculate_Interval_Calories(float distance_m, float weight_kg, float cot);
...
static void Callback_Timer(u32 timerId, void* param)
{
    ...
    switch(m_mqtt_state)
    {
        ...
        case STATE_MQTT_PUB:
        {
            ...
            // LOGIC: Calculate Distance and Calories for this interval
            float distance_meters = 0.0;
            
            if (g_is_first_fix) {
                // First valid fix, no previous point to compare
                distance_meters = 0.0;
                g_is_first_fix = FALSE;
            } else {
                // Calculate distance between current and last point
                distance_meters = Calculate_Distance_Meters(g_prev_lat, g_prev_lon, g_lat, g_lon);
            }

            // Update previous coordinates for next loop
            g_prev_lat = g_lat;
            g_prev_lon = g_lon;

            // Calculate calories for this specific interval
            Calculate_Interval_Calories(distance_meters, g_pet_weight, g_cot_c);
            
            // NOTE: g_speed removed from JSON as requested
            // NOTE: "cal" now contains only the interval consumption
            Ql_memset(mqtt_payload, 0, sizeof(mqtt_payload));
            
            Ql_sprintf((char*)mqtt_payload, 
                "{\"lat\":%d.%06d, \"lon\":%d.%06d, \"batt\":%d, \"cal\":%d.%02d, \"mode\":%d}", 
                (int)g_lat, (int)((g_lat - (int)g_lat)*1000000),
                (int)g_lon, (int)((g_lon - (int)g_lon)*1000000),
                batt,
                (int)g_interval_calories, (int)((g_interval_calories - (int)g_interval_calories)*100),
                g_mode
            );

            APP_DEBUG("PUB: %s (Dist: %.2fm)\r\n", mqtt_payload, distance_meters);
            msg_id++;
...
#define RADIO_TERRA 6371000.0 // Earth radius in meters
...
static float Calculate_Distance_Meters(float lat1, float lon1, float lat2, float lon2) {
    // If coordinates are identical (device didn't move), return 0
    if ((lat1 == lat2) && (lon1 == lon2)) return 0.0;

    double dLat = to_rad((double)(lat2 - lat1));
    double dLon = to_rad((double)(lon2 - lon1));
    double lat1_rad = to_rad((double)lat1);
    double lat2_rad = to_rad((double)lat2);

    double a = sin(dLat / 2) * sin(dLat / 2) +
               sin(dLon / 2) * sin(dLon / 2) * cos(lat1_rad) * cos(lat2_rad);
    
    double c = 2 * atan2(sqrt(a), sqrt(1 - a));
    double d = RADIO_TERRA * c;

    return (float)d;
}

static void Calculate_Interval_Calories(float distance_m, float weight_kg, float cot) {
    // Energy (Joules) = Distance (m) * Weight (kg) * COT
    double energy_joules = (double)distance_m * (double)weight_kg * (double)cot;
    
    // Convert to Kcal: 1 Kcal = 4184 Joules
    g_interval_calories = (float)(energy_joules / 4184.0);
}
...
static bool ParseRMC_Safe(char* rmc, float* lat, float* lon, float* speed) {
    ...
    if(lat_ptr && lon_ptr) {
        ...
        *lon = lon_d + (raw_lon - lon_d*100)/60.0;
        if(ew == 'W') *lon = -(*lon);
        // speed is parsed but not used in logic anymore
...
static void Parse_Attribute_Update(char* json_str) {
    ...
    // Parse Weight
    p = Ql_strstr(json_str, "pet_weight");
    if(p) {
    ...
    // Parse COT
    p = Ql_strstr(json_str, "cot_c");
    if(p) {
        char* val_ptr = Ql_strstr(p, ":");
        if(val_ptr) {
            g_cot_c = Ql_atof(val_ptr + 1);
            APP_DEBUG("NEW COT: %.2f\r\n", g_cot_c);
        }
    }
